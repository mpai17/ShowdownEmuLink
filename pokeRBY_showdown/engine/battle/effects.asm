...
.gotNumHits
	...
	inc a
.saveNumberOfHits
	; In Showdown mode, override hit count from server
	push af
	ld a, [wShowdownConnected]
	and a
	jr z, .useNormalHits
	pop af                      ; discard ROM's hit count
...
.useM2Hits
	ld a, [wSD_M2_Hits]
.gotOverrideHits
	and a                       ; safety: if 0, default to 1
	jr nz, .setHits
	ld a, 1
	jr .setHits
.useNormalHits
...
ConfusionSideEffect:
	ld a, [wShowdownConnected]
	and a
	jr nz, ConfusionSideEffectSuccess
	call BattleRandom
