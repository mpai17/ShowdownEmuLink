...
; In Showdown mode, repurposed as turn override data (10 bytes):
wLinkBattleRandomNumberList::
wSD_M1_DamageHi:: db  ; first mover's damage (high byte)
wSD_M1_DamageLo:: db  ; first mover's damage (low byte)
wSD_M1_Crit::     db  ; first mover crit flag (0 or 1)
wSD_M1_Miss::     db  ; first mover miss flag (0=hit, 1=miss)
wSD_M1_Effectiveness:: db ; first mover effectiveness byte
wSD_M2_DamageHi:: db  ; second mover's damage (high byte)
wSD_M2_DamageLo:: db  ; second mover's damage (low byte)
wSD_M2_Crit::     db  ; second mover crit flag
wSD_M2_Miss::     db  ; second mover miss flag
wSD_M2_Effectiveness:: db ; second mover effectiveness byte
ENDU
...
UNION
; Showdown handshake/control variables (overlays non-battle vars)
wSD_Flags::      db  ; bit0=playerCantMove, bit1=enemyCantMove, bit2=playerConfHitSelf, bit3=enemyConfHitSelf
                     ; bit4=M1 side effect should apply, bit5=M2 side effect should apply
                     ; bit6=player substitute should break, bit7=enemy substitute should break
wSD_Phase::      db  ; 0=idle, 1=player_selected, 2=turn_ready, 3=turn_executing
wSD_WhoFirst::   db  ; 0=player moves first, 1=enemy first
wSD_BattleEnd::  db  ; 0=ongoing, 1=won, 2=lost, 3=tie
wSD_M1_Hits::    db  ; multi-hit count for first mover (0 = not multi-hit)
